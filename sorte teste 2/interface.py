...
def fazer_login():
    ...
    if resultado_login:
        ...
        def reiniciar_para_login():
            ...
            # Limpa a tela antes de recriar o login
            for widget in root.winfo_children():
                widget.destroy()

            # Agora chama a função para mostrar a tela de login
            mostrar_tela_login()
...
def mostrar_tela_login():
    # Limpa qualquer widget anterior (certifique-se de que a tela de login será recriada)
    for widget in root.winfo_children():
        widget.destroy()

    # Configurações de título e geometria da tela de login
    root.title("Sistema de Login")
    root.configure(bg="gray")
    root.geometry("300x300")

    # Recria os widgets de login
    global entry_login, entry_senha  # Garantir que são globais para a função fazer_login
