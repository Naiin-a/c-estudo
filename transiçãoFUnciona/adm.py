...
def mostrar_usuarios(root, cpf_adm_executor, on_close_callback):
    from tkinter import messagebox
    from adm import excluir_usuario, editar_usuario, mostrar_adms, abrir_cadastro, abrir_cadastro_adm, exportar_logs_para_txt

    if not bd.conn:
        if not bd.conectar():
            messagebox.showerror("Erro Crítico", "Sem conexão com banco de dados.")
            return
    ...
    root.title("Painel do Administrador - Gerenciar Usuários")

    tk.Label(root, text="Gerenciar Usuários", font=("Helvetica", 14, "bold"), bg="gray").pack(pady=10, fill="x")
    ...
    for usuario in usuarios:
        ...
        texto = f"{nome} | CPF: {cpf_usuario} | Bloco: {bloco} | Ap: {numero_ap} | Email: {email}"
        tk.Label(frame, text=texto, anchor="w").pack(side="left", padx=5)

        tk.Button(frame, text="Editar", command=lambda c=cpf_usuario: editar_usuario(root, cpf_adm_executor, c)).pack(side="right", padx=2)
        tk.Button(frame, text="Excluir", command=lambda c=cpf_usuario: excluir_usuario(root, cpf_adm_executor, c)).pack(side="right", padx=2)
    ...
    tk.Button(frame_acao, text="Cadastrar Novo Usuário", command=lambda: abrir_cadastro(root, cpf_adm_executor)).pack(side="left", padx=5)
    tk.Button(frame_acao, text="Cadastrar Novo ADM", command=lambda: abrir_cadastro_adm(root, cpf_adm_executor)).pack(side="left", padx=5)
    tk.Button(frame_acao, text="Mostrar Logs", command=exportar_logs_para_txt).pack(side="left", padx=5)
    tk.Button(frame_acao, text="Gerenciar Administradores", command=lambda: mostrar_adms(root, cpf_adm_executor, on_close_callback)).pack(side="left", padx=5)

    tk.Button(root, text="Logout", command=on_close_callback).pack(pady=10)
